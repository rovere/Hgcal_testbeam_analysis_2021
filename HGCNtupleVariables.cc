...
int HGCNtupleVariables::getBIN(unsigned int skiroc, unsigned int channel) {
  if (channel % 2 != 0 || channel > 62 || skiroc > 3)
    return -1;
  else
...
std::vector<float> HGCNtupleVariables::getChi2Weights_EH(int beamEnergy_) {
  std::map<int, std::vector<float>>::iterator it =
      chi2_weights_EH.find(beamEnergy_);
...
std::vector<float> HGCNtupleVariables::getChi2Weights_H(int beamEnergy_) {
  std::map<int, std::vector<float>>::iterator it =
      chi2_weights_H.find(beamEnergy_);
...
std::pair<float, float> HGCNtupleVariables::dxy_alignment(int layer) {
  ...
  // std::map<std::pair<int,int>, std::pair<float,float> >::iterator it =
  // align_map.find(temp_find);
...
float HGCNtupleVariables::getNoise(std::pair<int, int> mod_chip) {
  ...
  if (it != noise_map.end()) {
    ...
    std::cout << "Value NOT found for Module = " << mod_chip.first
              << " & chip = " << mod_chip.second << std::endl;
  }
  return temp_noise;
}
