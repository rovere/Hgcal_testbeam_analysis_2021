...
void DataOverlay(string file_name)//_valid, string file_name_test, string file_name_data)
{
  ...
  sprintf(path2,"Results/Chi2_trimAhcal",file_name.c_str());
  TFile * inputfile = new TFile(hname,"READ");
  // TFile * inputfile1 = new TFile(hname1,"READ");
  // TFile * inputfile2 = new TFile(hname2,"READ");
  sprintf(hist_name,"Response_SSinEE");
  sprintf(hist_name1,"Resolution_SSinEE");
  //  sprintf(hist_name2,"resolution")
  sprintf(hist_name2,"Response_MipsInEE");
  sprintf(hist_name3,"Resolution_MipsInEE");
  sprintf(hist_name4,"Response_Total");
  sprintf(hist_name5,"Resolution_Total");
  
  sprintf(full_path1,"%s/overlay_resolution_chi2method1.png",path2);
  sprintf(full_path2,"%s/overlay_response_chi2method1.png",path2);
 ...
 TGraphErrors* h_resolution_data = (TGraphErrors*)inputfile->Get(hist_name5);
  ...
  h_resolution_valid->SetMarkerSize(1.5);
  ...
  h_resolution_test->SetMarkerSize(1.5);
  ...
  h_resolution_data->SetMarkerSize(1.5);
  ...
  h_response_valid->SetMarkerSize(1.5);
  ...
  h_response_test->SetMarkerSize(1.5);
  ...
  h_response_data->SetMarkerSize(1.5);
  h_response_data->SetMarkerStyle(kFullSquare);
  TMultiGraph* mg = new TMultiGraph();
  TMultiGraph* mg1 = new TMultiGraph();

  TCanvas *canvas_n1 = new TCanvas(hist_name, hist_name,800,800,1400,1400);
  ...
  legend->SetHeader("","C");
  legend->AddEntry(h_resolution_valid,"SSinEE","p");
  legend->AddEntry(h_resolution_test,"MipsInEE","p");
  legend->AddEntry(h_resolution_data,"Total","p");
  ...
  h_resolution_test->SetMaximum(0.6);
  ...
  TCanvas *canvas_n2 = new TCanvas(hist_name1, hist_name1,800,800,1400,1400);
  ...
  gPad->Modified();
  gPad->Update();
  h_response_test->GetHistogram()->GetYaxis()->SetRangeUser(0.5,1.4);
  h_response_test->Draw("ALP");
   auto legend1 = new TLegend(0.7,0.1,0.9,0.3);
  legend1->SetHeader("","C");
  legend1->AddEntry(h_resolution_valid,"SSinEE","p");
  legend1->AddEntry(h_resolution_test,"MipsInEE","p");
 legend1->AddEntry(h_resolution_data,"Total","p");
 gStyle->SetLegendTextSize(0.05);
  mg1->Draw("LP");
